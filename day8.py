...
def catagorize(sigs):
    ...
    for sig in sigs:
        r = toset(sig)
        a[len(r)] = a.get(len(r),[]) + [r]
    return a

def readOut1(instr):
    (fs,os) = instr.split("|")
    signals = list(filter(lambda x : len(x) > 0, fs.split(" ")))
    tests = list(filter(lambda x : len(x) > 0, os.split(" ")))

    sigs = catagorize(signals)
    vals = {}
    one = sigs[2].pop(0)
    seven = sigs[3].pop(0)
    four = sigs[4].pop(0)
    ...
    tests = map(lambda x: set(list(x)),tests)
    test_cor = filter(lambda x: x in [one,seven,four,eight],tests)
    return len(list(test_cor))

def reverse(hs,k):
    for i,j in hs.items():
        if j == k:
            return i
    print("couldnt find: ",k,"in",hs)
...
def transform(instr):
    ...
    for i in sigs[6]:
        if len(seven.union(i)) == 7:
            six = i

    sigs[6].remove(six)
    
    for i in sigs[6]:
        if len(four.union(i)) == 6:
            nine = i
    ...
    three = None
    for i in sigs[5]:
        if len(i.union(nine)) == 7:
            five =i
        if len(i.union(one)) == 5:
            three = i
    ...
    vals = {
        ...
        0: zero}
    #print(vals)
    a = list(map(lambda x: reverse(vals,set(list(x))),tests))
...
def readOut2(instr):
    ...
    a = list(map(lambda x: reverse(vals,x),tests))
    return a[0]*1000 + a[1] * 100 + a[2] * 10 + a[3]

  
data = '''eb cbgfae cabdf fedab efb adgcef cbgaefd egdb dbgefa eafgd | dfbae be gdafe gcefab
...
data2 = '''be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |fdgacbe cefdb cefbgd gcbe
...
print(sum(map(readOut1,data.split("\n"))))
print(sum(map(transform,data.split("\n"))))
